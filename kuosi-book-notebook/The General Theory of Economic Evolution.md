# [输入给 NotebookLM / AI 图像生成引擎的系统提示/背景信息]

## 文档用途
用于和读书思考爱好者的交流（Deck）。

## 使用场景
「开源之道」日常交流 / 「OSCAR·开源之书·共读」meetup 分享

## 听众画像
思考者，阅读者，善于交流者。对制度经济学、开源文化、人类学有基本的认知框架。

## 讲者视角
思维活跃者、独立开源研究者、协作机制设计师（社会黑客）。

## 核心诉求
展示人类的思维活动，渴望获得交流。

## 核心基调
dark academic tone, Intellectual Visual System, art taste.

## 视觉风格关键词
- 包豪斯几何构成（Bauhaus geometric composition）
- 深普鲁士蓝 #1B3B6B 作为强调色——代表智识深度与学术传统
- 暖白底色 #F5F0E8，羊皮纸质感——书本与经卷的联想
- 极简主义排版，大字号标题，宽松行距
- 深色学术风格（dark academic），理性、克制、智识感
- 几何抽象元素——圆、方、线作为装饰，不依赖图标
- 书籍封面/古籍页面纹理作为背景元素
- 深色学术风格（dark academic）整体调性

## 内容结构约定
- 每张 slide 包含：视觉隐喻（一句话描述画面）+ 显示要点（3-5 个 bullet point）
- 封面 slide 包含：主标题、副标题、讲者信息、时间/地点，以及元数据（书名·作者·出版社·ISBN）
- 结尾 slide 包含：讨论问题或延伸思考

---

## Slide 1: Cover
* **主标题：** 规则之影：经济演化与知识秩序的重构
* **副标题：** 从制度变迁视角审视经济知识生长与规则协调的复杂系统
* **视觉隐喻：** 昏暗图书馆中，泛黄羊皮卷自动翻页，墨迹化作金色机械藤蔓缠绕沙漏，背后阴影浮现齿轮构成的秩序之树，烛光摇曳，尘埃飞舞。
* **讲者信息：** 「开源之道」·适兕
* **时间/地点：** 2007-10
* **元数据：** The General Theory of Economic Evolution | Dopfer, Kurt/ Potts, Jason

## Slide 2: 书摘——rules is everything

* 视觉隐喻：
  * 超写实发光金色几何网格由野生藤蔓编织而成，秩序取代混乱，象征规则协调知识增长，避免回归野兽状态。
* 显示要点：
  * **书摘：** All economic knowledge and the growth of knowledge can be analyzed as a process of coordination and change in rules. rules，without it we live with animals.

---
> 迁移自豆瓣读书笔记：https://book.douban.com/people/johnwoo_lee/annotation/
> 视觉隐喻待 LLM 根据内容生成；封面标题副标题待生成。
