# [输入给 NotebookLM / AI 图像生成引擎的系统提示/背景信息]

## 文档用途
面向开源经济学讲座的 Deck：以新制度经济学视角，系统梳理开源软件的生产、许可、商业模式、劳动力市场、交易成本、组织治理、文化与政治经济学等议题，帮助思考者从制度、协作与激励机制理解数字时代的开源现象。

## 使用场景
「开源之道」日常交流 / 新制度经济学视角下的开源讲座

## 听众画像
思考者，阅读者，善于交流者。对制度经济学、开源文化、人类学有基本的认知框架。

## 讲者视角
思维活跃者、独立开源研究者、协作机制设计师（社会黑客）。

## 核心诉求
展示人类的思维活动，渴望获得交流。

## 核心基调
dark academic tone, Intellectual Visual System, art taste.

## 视觉风格关键词
- 包豪斯几何构成（Bauhaus geometric composition）
- 深普鲁士蓝 #1B3B6B 作为强调色——代表智识深度与学术传统
- 暖白底色 #F5F0E8，羊皮纸质感——书本与经卷的联想
- 极简主义排版，大字号标题，宽松行距
- 深色学术风格（dark academic），理性、克制、智识感
- 几何抽象元素——圆、方、线作为装饰，不依赖图标
- 书籍封面/古籍页面纹理作为背景元素
- 深色学术风格（dark academic）整体调性

## 内容结构约定
- 每张 slide 包含：视觉隐喻（一句话描述画面）+ 显示要点（3-5 个 bullet point）
- 封面 slide 包含：主标题、副标题、讲者信息、时间/地点
- 结尾 slide 包含：讨论问题或延伸思考

---

# economic-of-open-source-lecture-0

<!-- Part 1 of 4, slides 1-15 / 48 -->

---
# economic-of-open-source-lecture-0

<!-- Part 1 of 4, slides 1-15 / 50 -->

---
# economic-of-open-source-lecture-0

<!-- Part 1 of 4, slides 1-15 / 51 -->

---
# economic-of-open-source-lecture-0

<!-- Part 1 of 4, slides 1-15 / 51 -->

---

# the-open-source-way-2-lijiansheng-shanghai.pptx

<!-- Source: ~/onedrive/开源之书/the-open-source-way-2-lijiansheng-shanghai.pptx -->

<!-- Part 2/3, slides 16-30 / 33 -->

---


## Slide 16
- 关于多样性与包容性
- 兼容并蓄
- 降低各式门槛
- 克制
- 理性
- 观念塑造世界
- If your community doesn’t look like this world, it won’t last long in this world.
- ---

---

## Slide 17
- 关于一些共同体构建之术与器
- 谁、什么、哪里、什么时间、为什么、如何？放在哪里都适用
- 电子邮件、IRC、Slack 到GitHub 的沟通变化
- 项目介绍、新手入门、FAQ、项目目标
- Issue track 事情真正发生的地方
- 维护公民话语权、社会契约、行为准则、
- ---

---

## Slide 18
- 关于Contributing的多种角色
- 销售和业务发展
- 市场营销
- 战略技能
- 数据科学
- 图形设计
- 项目管理和计划管理技能
- 产品管理
- 法律法务
- 人力资源/拉人
- ---

---

## Slide 19
- 其实开源共同体中最不缺的就是开发者。当然，优秀的开发者哪里都稀缺。
- 适兕
- ---

---

## Slide 20
- 参与到开源共同体的动机或激励
- ---

---

## Slide 21
- 关于动机的理解和运用
- 马斯洛金字塔模型是常见的法则
- 要重视人的内在，尤其是其心理过程，这是人类难能可贵的品质
- 外部激励的重要性，同样不可或缺。
- 也就是说这会透视一个社会的文明程度（民主社会表现更佳）
- ---

---

## Slide 22
- Contributor 增长
- 这是所有认为开源是希望的“一夜暴富”的想法
- 从项目到共同体：事物与人类社会
- 友善、帮助、以人为本、构建关系、
- ---

---

## Slide 23
- Building 共同体
- Building leadership and preparing for leadership changes.
- Recruiting, training, and mentoring members, including diverse roles and demographic groups.
- Decision-making and conflict resolution.
- Upholding positive norms.
- Recognition and rewards.
- Running effective meetings.
- ---

---

## Slide 24
- Onboarding
- 可以比喻为公司新人入职/学校新生入学
- 将应该交待清楚的事情都交待清楚
- Contribute 路径也是蛮重要的
- ---

---

## Slide 25
- Mentor 计划
- 全方位受益
- Mentor 的方式：Educator、Encourager、Listener、Finder/Researcher、Connector
- Mentor 计划/原则：
- Mentorship is an act, experience, and opportunity to share what you can, when you can, how you can.
- ---

---

## Slide 26
- 项目和共同体的治理（ Governance）
- 有人的地方就有江湖！（关于武侠小说中透露的文化）
- 所谓的治理：即是规则和习俗的规制。
- 角色的定义与诠释
- 政策和流程的制定
- 为什么要Governance？
- When your project’s rules are clear, contributors can engage with confidence.
- ---

---

## Slide 27
- 一定要让治理明确化
- 成功的开源项目这点是必修项
- 透明度
- 培养强烈的信任感
- 2018年的Kubernetes 的治理推进堪称经典
- ---

---

## Slide 28
- 基于共同体的项目的成长路径
- 创始人，1～2
- 早期，5～10人
- 中期，10～15人
- ---

---

## Slide 29
- 本土有着完全不同的情形
- ---

---

## Slide 30
- 合作的进化
- 单一的小团队
- 多个不同的利益团体加入
- ---

---

