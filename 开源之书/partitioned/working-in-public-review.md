# [输入给 NotebookLM / AI 图像生成引擎的系统提示/背景信息]

## 文档用途
面向开源经济学讲座的 Deck：以新制度经济学视角，系统梳理开源软件的生产、许可、商业模式、劳动力市场、交易成本、组织治理、文化与政治经济学等议题，帮助思考者从制度、协作与激励机制理解数字时代的开源现象。

## 使用场景
「开源之道」日常交流 / 新制度经济学视角下的开源讲座

## 听众画像
思考者，阅读者，善于交流者。对制度经济学、开源文化、人类学有基本的认知框架。

## 讲者视角
思维活跃者、独立开源研究者、协作机制设计师（社会黑客）。

## 核心诉求
展示人类的思维活动，渴望获得交流。

## 核心基调
dark academic tone, Intellectual Visual System, art taste.

## 视觉风格关键词
- 包豪斯几何构成（Bauhaus geometric composition）
- 深普鲁士蓝 #1B3B6B 作为强调色——代表智识深度与学术传统
- 暖白底色 #F5F0E8，羊皮纸质感——书本与经卷的联想
- 极简主义排版，大字号标题，宽松行距
- 深色学术风格（dark academic），理性、克制、智识感
- 几何抽象元素——圆、方、线作为装饰，不依赖图标
- 书籍封面/古籍页面纹理作为背景元素
- 深色学术风格（dark academic）整体调性

## 内容结构约定
- 每张 slide 包含：视觉隐喻（一句话描述画面）+ 显示要点（3-5 个 bullet point）
- 封面 slide 包含：主标题、副标题、讲者信息、时间/地点
- 结尾 slide 包含：讨论问题或延伸思考

---


<!-- Part 1 of 4, slides 1-15 / 48 -->

---

<!-- Part 1 of 4, slides 1-15 / 50 -->

---

<!-- Part 1 of 4, slides 1-15 / 51 -->

---

<!-- Part 1 of 4, slides 1-15 / 51 -->

---

# working-in-public-review.pptx

<!-- Source: ~/onedrive/开源之书/working-in-public-review.pptx -->

<!-- Total: 24 slides -->

<!-- Part 1/2, slides 1-15 / 24 -->

---


## Slide 1
- 现代开源软件探秘
- 《在所有人看得见的地方工作：开源软件的开发与维护》读书分享

---

## Slide 2
- 自行修炼的“博士”

---

## Slide 3
- 关注重要而缺乏支持的社会现象
- Nadia Eghbal
- Reimagining the phd
- Phase I: 2015-2016: Discovery, exploration, initial research, hypothesis generation (funded by the ford foundation)
- Phase II: 2016-2018: Experience, experimenting, hypothesis testing (working at github)
- Phase III: 2018-2020: Refining, summarizing, consolidation (funded by protocol labs)

---

## Slide 4
- 主要成果和材料
- Roads and Bridges: The Unseen Labor Behind Our Digital Infrastructure,
- Open Source Guides
- Working in Public: The Making and Maintenance of Open Source Software
- awesome-maintainers: Talks, blog posts, and interviews about the experience of being an open source maintainer
- https://nadiaeghbal.com/oss/  发现更多：Podcast、blog etc.

---

## Slide 5
- 路与桥：那些被忽视的现代软件基础设施

---

## Slide 6
- 终于有人指出房间里大象的存在
- 请环顾四周，那个服务使用到了开源软件
- 这些开源软件背后的作者是否获得了充足的报酬？社会是否给予他们回报？
- 探究的历程：NTP server 作者、Linux Kernel 的后继者……
- 开源软件的可持续性面临的挑战和可能的解决方案

---

## Slide 7
- There is a need for support for the free public infrastructure….People scream if their clocks are off by a second. They say, ‘yes, we need you, but we can’t give you any money. – Harlan Stenn, Network Time Protocol  维护者

---

## Slide 8
- Many of us have a five at the beginning of our ages, while a couple are approaching their 60s. So at some point, we as a community need to start thinking about generational change.……It turns out it's really hard to find people who are maintainers
- Linus Torvalds 在2020年在线北美开源峰会上如此说

---

## Slide 9
- 换一个视角看开源

---

## Slide 10
- 为什么要写这样一本非虚构类的书？
- 关注被大众忽略但却非常重要的社会现象
- 为什么GitHub 会成为 social coding 的平台？
- 开源项目的人员构成是什么样子的？
- 这些所谓的开源人都有哪些角色？
- 开源的经济来源的合理性在哪里？
- 开源项目从开发到交付都有哪些开销？
- 未来在哪里？

---

## Slide 11
- 介绍
- 信息
- 重温 Tim Berners Lee 博士的WWW论文
- 开发者在公开的地方工作，要么被有钱的公司发现，要么就销声匿迹
- 很多人希望获得金钱上的资助
- 从熟悉的同事到完全的陌生人： 开源的社会协作（全新的社会意义）
- 维护者的苦恼：从review代码到沟通交互
- Solo creator 的崛起

---

## Slide 12
- 第一章：Github 作为平台
- 开源历史的演进：现代的开发者根本不在乎（意识形态、理想、自由）
- Git 和 github 究竟是谁成就了谁？
- GitHub 的根本优势：易用、易分享、易发现他人的代码
- JavaScript 项目占尽开源的优势
- GitHub as Social Coding！

---

## Slide 13
- 第二章：开源项目的结构
- 参与开源，不仅是一个技术活，还是一个社交活，而且还是在公开的场合下进行。
- Open source community 的消失，与 Urban community 的崛起（复杂到比喻为城市化）
- 开发者是一个需要经过训练的群体：尽管是向全人类公开，且可修改的。
- Reputation is important！
- 开源项目的涉及范围

---

## Slide 14
- 第二章：续
- 开源项目的类型

---

## Slide 15
- 第三章：角色、激励和关系
- 源代码和软件的商业化，取决于人们的价值认知。
- 开源项目作为公共物品
- 内在激励与外在激励
- 维护者、贡献者、用户
- Casual contributor（即兴贡献者）

---

