# [输入给 NotebookLM / AI 图像生成引擎的系统提示/背景信息]

## 文档用途
用于和读书思考爱好者的交流（Deck）。

## 使用场景
「开源之道」日常交流 / 「OSCAR·开源之书·共读」meetup 分享

## 听众画像
思考者，阅读者，善于交流者。对制度经济学、开源文化、人类学有基本的认知框架。

## 讲者视角
思维活跃者、独立开源研究者、协作机制设计师（社会黑客）。

## 核心诉求
展示人类的思维活动，渴望获得交流。

## 核心基调
dark academic tone, Intellectual Visual System, art taste.

## 视觉风格关键词
- 包豪斯几何构成（Bauhaus geometric composition）
- 深普鲁士蓝 #1B3B6B 作为强调色——代表智识深度与学术传统
- 暖白底色 #F5F0E8，羊皮纸质感——书本与经卷的联想
- 极简主义排版，大字号标题，宽松行距
- 深色学术风格（dark academic），理性、克制、智识感
- 几何抽象元素——圆、方、线作为装饰，不依赖图标
- 书籍封面/古籍页面纹理作为背景元素
- 深色学术风格（dark academic）整体调性

## 内容结构约定
- 每张 slide 包含：视觉隐喻（一句话描述画面）+ 显示要点（3-5 个 bullet point）
- 封面 slide 包含：主标题、副标题、讲者信息、时间/地点
- 结尾 slide 包含：讨论问题或延伸思考

---

# copyleft-right-to-include-and-os

<!-- Source: /home/lee/onedrive/开源之书/2023-11/copyleft-right-to-include-and-os.pptx -->

<!-- Total: 22 slides, Part 2/2 (slides 16-22) -->

---

## Slide 16: 谈判而不是对抗
* 视觉隐喻：
  * 沃顿商学院最受欢迎的谈判课书籍封面
* 显示要点：
  * 洞悉自己的诉求
  * 利用对方的规则
  * 达到自己的目的
  * 也要实现他人的目的
  * Copyright /Copyleft 是一种选择

---

## Slide 17: 容忍性使用
* 视觉隐喻：
  * 粉色背景上的彩色波浪形3D带子
* 显示要点：
  * “我们都是盗版的一代”
  * Web2.0 的 “ReMixed” 时代
  * 知识财产权的无法enforcing 性
  * 改变了产业结构～
  * 抖音&TikTok  加速了数字内容的“容忍性使用”

---

## Slide 18: 吴修铭
* 视觉隐喻：
  * 注意力经济：注意力变生意
* 显示要点：
  * Timothy Wu
  * Julius Silver Professor of Law, Science and Technology
  * 关注大众创新和科技发展、反垄断
  * https://www.law.columbia.edu/faculty/timothy-wu

---

## Slide 19: 弃权/公有领域
* 视觉隐喻：
  * 散乱堆积的白色立体字母。
* 显示要点：
  * Sqlite 的放弃版权，进入public domain
  * 集体创新项目的新选择
  * www 放弃 GPL，选择放弃所有权
  * 被一部分人认为的开源许可

---

## Slide 20: 容他权
* 视觉隐喻：
  * 一根针穿过彩虹色的毛线
* 显示要点：
  * 排他权在知识财产法的历史上起到了很大的作用
  * 数字时代带来了新挑战：复制成本为零
  * 不再受限于个体和少数人的作品，而是大规模基于互联网的协作

---

## Slide 21: 开源的贡献与未来
* 视觉隐喻：
  * 蓝紫色发光线条组成的网格波浪
* 显示要点：
  * 技术的本质和路径依赖选择了开源
  * 互联网和开源彼此成就
  * 数字世界的重要基础设施
  * 机器学习的重要材料
  * 财产权的鉴定与开发者的回报产生了扭曲
  * 现在和未来仍然需要财产权做出必要的举措

---

## Slide 22: ————Gnu general public license  前言
* 视觉隐喻：
  * 深色几何方块护泛黄书页，双钥开启自由权。
* 显示要点：
  * ————Gnu general public license  前言

---
